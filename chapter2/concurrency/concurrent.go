...
func writeAMail(wg *sync.WaitGroup) {
	fmt.Println("Wrote 1/3rd of the email.")
	go continueWritingMail1(wg)
}

func continueWritingMail1(wg *sync.WaitGroup) {
	fmt.Println("Wrote 2/3rd of the email.")
...
var listOfTasks = []func(wg *sync.WaitGroup){
	makeHotelReservation, bookFlightTickets, orderADress, payCreditCardBills, writeAMail, listenToAudioBook,
}
